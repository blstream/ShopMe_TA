Feature: PRODPFIP-26 Re-search phrases
  ...
  Background:
    ...
    # | service name | category | user name | user email      | user phone | user info | base description | base price | extended description | extended price | extra description | extra price | province           | city     |
      | zmywanie     | budowa   | test      | test@domain.com | 888555222  | test      | test             | 10         | test                 | 20             | test              | 30          | zachodniopomorskie | Szczecin |
      | pranie       | budowa   | test      | test@domain.com | 888555222  | test      | test             | 10         | test                 | 20             | test              | 30          | zachodniopomorskie | Szczecin |
      | mycie        | budowa   | test      | test@domain.com | 888555222  | test      | test             | 10         | test                 | 20             | test              | 30          | zachodniopomorskie | Szczecin |
  ...
  Scenario: Adding new offer containing phrase in the title and searching for that phrase again
    ...
    When I add services
    # | service name     | category | user name | user email      | user phone | user info | base description | base price | extended description | extended price | extra description | extra price | province           | city     |
      | mycie samochodów | budowa   | test      | test@domain.com | 888555222  | test      | test             | 10         | test                 | 20             | test              | 30          | zachodniopomorskie | Szczecin |
    And I enter a searching phrase "mycie" into the search field
    And I click the search button
    Then search results are visible
